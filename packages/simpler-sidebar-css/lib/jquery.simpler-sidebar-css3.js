...
function _objectSpread(target) {
  for (var i = 1; i < arguments.length; i++) {
    ...
    i % 2
      ? ownKeys(new Object(source), !0).forEach(function (key) {
    ...
    return this.each(function () {
      ...
      $sidebar.attr(sidebarAttrOpen, configs.open).css(
        ...
        _defineProperty(
          _$sidebar$attr$css,
          configs.align,
          configs.open ? 0 : -setSidebarWidth(windowWidth)
        ),
        _defineProperty(
          _$sidebar$attr$css,
          'transition',
          ''
            .concat(configs.align, ' ')
            .concat(configs.animation.duration / 1000, 's ')
            .concat(configs.animation.easing)
        ),
        _$sidebar$attr$css)
      );
